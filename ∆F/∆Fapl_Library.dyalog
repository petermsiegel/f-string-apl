⍝ ∆Fapl_LibSC.dyalog      (UPDATE_TIME: '2025-11-21T15:05:33') 
:Namespace libUtil 
...
⍝      ulNm ulNs are copies of ##.ulNmG ##.ulNsG, 
⍝      normally name of ##.ûserLib and ref to it.
...
⍝ Auto: u@nsNm←  ∇ str@CV verbose@B auto@B 
⍝    str starts 1 char after £ or `L. 
⍝    extern: ulNm, ulNs  
⍝  See steps below. 
⍝ Returns: ulNm (the stringified library namespace), no matter what.  
  ∇ u← Auto (str verbose auto) ;t ;⎕IO ;⎕ML  
    ⎕IO ⎕ML←0 1
    u← ulNm                                            ⍝ Return ulNm no matter what!
    :If   ~auto                                        ⍝ Disable auto mode if auto=0.
    :OrIf 0=  ≢str                                     ⍝ Empty str? Done
    :OrIf '.'≠ ⊃str← NoLB str                          ⍝ No dot after £? Done
    :OrIf ~⍙A∊⍨ ⊃str← NoLB 1↓str                       ⍝ Word after '.' start of APL name? Done
    :OrIf 0≠ulNs.⎕NC nm← str↑⍨ t← +/∧\str∊ ⍙AD         ⍝ Word isn't an APL name? Done
    :OrIf '←'= ⊃NoLB t↓str                             ⍝ It's a simple assignment? Done
    :Else 
  ...
  ⍙LoadObj← { 
    ...
    ⍝ ∘ When ⎕FIX is applied to ¨fi¨, ¨nm¨ must be among the names listed as ⎕FIXed. 
      SCF_FixByType← { nm fi←⍺ ⍵ ⋄ sfx← ⊃⌽⎕NPARTS fi   
        '.aplf' '.aplo' '.apln'∊⍨ ⊂sfx: rcER rcOK⊃⍨ (⊂nm)∊ 2 ulNs.⎕FIX fi 
            Set← ulNs {⍺⍺⍎ ⍺,'←⍵'}
        '.apla'≡ sfx: rcOK⊣ nm Set ##.AN2Apl ⊃⎕NGET  fi 1 
        '.txt' ≡ sfx: rcOK⊣ nm Set ⊃⎕NGET fi 1  
            jOpts← ('Dialect' 'JSON5')('Compact' 0)('Null' ⎕NULL)                                     
        '.json'≡ sfx: rcOK⊣ nm Set ⎕JSON⍠jOpts ⊃⎕NGET fi 1  
...
⍝      actual ref: ##.ûserLib, local ref (alias): ulNs, local name: ulNm.
  ∇ {libNs}← UserLibMax (libNm libNs)
    ⍝ external: ulNm ulNs 
      (ulNm ulNs)← libNm libNs
      ulNs.⎕DF '£=[',ulNm,']'
  ∇

⍝ =========================================================================
⍝ EXECUTIVE
  UserLibMax ##.(ulNmG ulNsG)
  LoadParms 1 1 0
